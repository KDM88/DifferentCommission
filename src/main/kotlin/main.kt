...
fun limit(transferAmount: Int, previousTransfer: Int = 0, userType: String = "VK Pay"): String {
    val message = when (userType) {
        ...
        ACCOUNT_TYPE_MASTER, ACCOUNT_TYPE_MAESTRO, ACCOUNT_TYPE_VISA, ACCOUNT_TYPE_MIR -> {
            if ((transferAmount + previousTransfer) >= 150_000) MESSAGE_DAY
            else if ((transferAmount + previousTransfer) >= 600_000) MESSAGE_MONTH
            else MESSAGE_COMMISSION
        }
...
fun commissionCalculate(transferAmount: Int, previousTransfer: Int = 0, userType: String = "VK Pay"): Int {

    val commissionSum = when (userType) {
        ACCOUNT_TYPE_VK_PAY -> 0
        ACCOUNT_TYPE_MASTER, ACCOUNT_TYPE_MAESTRO -> when (transferAmount) {
            in 75_000..150_000 -> ((transferAmount * 0.006 * 100) + 2_000).toInt()
            else -> 0
        }
